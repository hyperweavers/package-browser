...
export class PackageService {
  ...
  private corsAnywhereUrl: string = "https://cors-proxy.fringe.zone/";
  ...
  searchByKeyword(
    ...
    return this.http
      ...
      .pipe(map(this.mapPackages.bind(this)));
  }
  ...
  searchByAuthor(
    ...
    return this.http
      .get(
        `${url}&text=author:${author}&from=${from}&size=${this.packagesPerPage}`
      )
      .pipe(map(this.mapPackages.bind(this)));
  }
  ...
  getPackages(sortBy: string, page?: number): Observable<Package[]> {
    ...
    return this.http
      ...
      .pipe(share(), map(this.mapPackages.bind(this)));
  }
  ...
  private mapPackage(response: Response, packageName: string): Package {
    ...
    if (res.error === undefined && res.reason === undefined) {
      const repoUrl = res.repository.url
        ? this.sanitizeUrl(res.repository.url)
        : "";

      const dependencyList = [];

      for (const dependency in res.versions[res["dist-tags"].latest]
        .dependencies) {
  ...
  private toPackage(obj: any): Package {
    ...
    const pkg = <Package>{
      ...
      authorName:
        obj.package.author && obj.package.author.name
      ...
      repoUrl: obj.package.links.repository
        ? this.sanitizeUrl(obj.package.links.repository)
        : "",
      npmUrl: obj.package.links.npm
        ? obj.package.links.npm
        : this.npmPkgUrl + obj.package.name,
  ...
  private isGithubUrl(url: string): boolean {
    return (
      url.slice(0, "http://github.com".length) === "http://github.com" ||
        ...
        if (absoluteUrlRegExp.test(relativePath) === false) {
          if (repoUrl !== "") {
            ...
            readme = readme.replace(
              url,
              "(" + repoUrl + "/raw/master/" + relativePath + ")"
            );
          }
        }
      }
